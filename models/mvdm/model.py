...
def Model1_B():
    ...
    the SN/SP subsystems as well. For exploration only
    """
    M1_B = Model()
    return M1_B
...
def TAN_LearningRule(p, context):
    ...
    X  = p.groupFrom.activations
    Y1 = p.groupTo.activations
    if context.GetParameter("TRACE_TAN") and (Y1==HB).all():
        print "**** Hard bound found ****"
    Y1 = np.maximum(Y1, HB)
    Y0 = p.groupTo.GetHistory()[-1]
    # Added internal parameter 2011-11-28
    daT = context.GetParameter("daT")
    dY = Y1-Y0
...
def Model3():
    """
    A version of Model 2 with Da-->TAN projections of the opposite
    sign (excitatory instead of inhibitory).
    """
    M2      = Model2()    # Get a base model 2
    tan     = M2.GetGroupByName("TAN")
    da      = M2.GetGroupByName("Da")
    da2tan  = M2.GetProjectionsBetweenGroups(da, tan)[0]
    
    # Invert the sign of the W matrix. 
    da2tan.weights = -1*da2tan.weights
    M2.SetParameter("daT", -1*dat)
    M2.name = "Model3"
    return M2
...
def Model4():
    """
    Version of Model2 with separate time and state. Requires special handling
    of state information.
    """
    m3    = Model3()
    state = Group(3, name="State")
    
    for n in ("SN", "SP", "TAN", "PVi", "LVe", "LVi"):
        x = m3.GetGroupByName(n)
        p = state.ConnectTo(x)
        p.weights=np.zeros(p.weights.shape)+0.001
        p.mask=np.zeros(p.weights.shape)
...
def gg(m, n1):
    """GetGroup--returns a group from a given circuit"""
    return m.GetGroupByName(n1)

def gp(m, n1, n2):
    """GetProjectiions---returns a projection from a given model"""
